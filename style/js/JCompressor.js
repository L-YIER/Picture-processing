...
function ImgPreview(filedata) {
    ...
    if (isMultiple) {
        ...
        opts.PreviewContainer.style.display = "grid";
        opts.PreviewContainer.style.gridTemplateColumns = "repeat(auto-fill, minmax(150px, 1fr))";
        opts.PreviewContainer.style.gap = "10px";
        opts.PreviewContainer.style.padding = "10px";
        opts.PreviewContainer.style.background = "#f5f5f5";
        opts.PreviewContainer.style.borderRadius = "5px";
        opts.PreviewContainer.style.width = "100%";
        opts.PreviewContainer.style.maxWidth = "800px";
        ...
        filedata.forEach(function(item, index) {
            ...
            deleteBtn.onclick = function() {
                container.remove();
                if (fileArr && fileArr[index]) {
                    fileArr.splice(index, 1);
                    fileBase64Arr.splice(index, 1);
                }
            };

            container.appendChild(img);
            container.appendChild(deleteBtn);
            opts.PreviewContainer.appendChild(container);
        });
    } else {
        if (isimgtag) {
            opts.PreviewContainer.src = filedata;
            opts.PreviewContainer.style.maxWidth = "100%";
            opts.PreviewContainer.style.height = "auto";
        } else {
